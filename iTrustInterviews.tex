...
Unfortunately, however, research suggests that many developers may not be using such static analysis tools, in part because they have difficulty interpreting the output~\cite{johnson2013don}. 
...
For each category, we list some more frequent or interesting distinct questions asked and the number of participants that asked a variation of that question in parentheses; a full list of distinct questions can be found online.\footnote{\url{www4.ncsu.edu/~bijohnso/questions.pdf}} 
For each category, we discuss the common thread that ties questions in that category together.
We list each distinct question we extracted with the number of participants that asked that question in parentheses.  
...
For example, when exploring the Path Traversal vulnerability, participants sought the locations in the code where the method was being called and where the path parameter was set.
...
Most of these questions occurred in the context of assessing the Predictable Random and Servlet Parameter vulnerability.
