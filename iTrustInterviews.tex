...
Finally, we describe the strategic successes and failures we observed, and how future tools can leverage our findings to encourage better strategies.	
...
Unfortunately, despite their availability, research suggests that developers do not use static analysis tools, partially because tools do not provide the information needed to resolve defects~\cite{johnson2013don}. 
...
Our work addresses this problem by advancing our understanding of how developers use a security-focused static analysis tool resolve security defects, including their information needs, defect resolution strategies, and assumptions.
To our knowledge, no prior study has specifically investigated developers' information needs, strategies, or assumptions while using such a tool. 
...
It focused solely on the categorization of questions developers asked while resolving security vulnerabilities. 
In this work, we also consider the approaches developers take to answer those questions.
This paper makes the following additional contributions:

\begin{itemize}
	\item An analysis of developers' defect resolutions strategies. (Section \ref{sec:results})
	\item Identification of the common assumptions that undermined those strategies. (Section \ref{sec:assumptions})
	\item Visions for new tool interfaces based on the strategy and assumption analysis. (Section \ref{sec:discussion})	
\end{itemize}
...
These strategies for determining the set of attacks to consider were prone to two types of failures. 
...
Password vs form input.

Some strategies work in one context but fail in another (java array bounds checking?)
