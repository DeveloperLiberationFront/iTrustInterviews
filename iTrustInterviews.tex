...
\author{Justin~Smith, Brittany~Johnson, Emerson~Murphy-Hill, Bill~Chu, and Heather~Richter~Lipford%
		\IEEEcompsocitemizethanks{
			...
			\IEEEcompsocthanksitem B. Johnson is with University of Massachusetts Amherst, Amherst, MA 01003. E-mail: bjohnson@cs.umass.edu
...
Using a card sort methodology, two authors sorted 559 questions into 17 categories. 
...
Including questions that were only identified by one reviewer, this process resulted in a total of 559 questions.
...
%Because it was our intention to capture developers' overall defect resolution strategies, our analysis strives to summarize all non-obligatory, goal directed actions.
...
Ultimately, we created a new notation for representing strategies, called \textit{strategy trees}.
...
%
%how the authors came up with the strategy tree, 
%Base on the idea of a GOMS model and attack tree
%-Why wasn't GOMS sufficient...
...
Whenever we observed the participant making a verifiably incorrect statement, abandoning a strategy without achieving the desired result, or overlooking information they sought, it was marked as a failure.
...
%Although this introduces a potential confirmation bias threat, we were compelled to make this choice for the following two reasons.
...
Specifically, participants wondered where the potentially insecure random number generator was being used and whether it was employed to generate sensitive data like passwords.
...
These studies give us insight into the needs of developers during general programming tasks.
Our work examines information needs during a more specific programming task, diagnosing security vulnerabilities with static analysis.
We identified information needs unique to security, such as needs pertaining to attacks and vulnerabilities, as well as previously known information needs with novel security implications. 
Throughout Section \ref{sec:results} we have discussed how the information needs identified in previous studies overlap with the information needs we identified.
...
Program comprehension refers to the process by which developers make sense of unfamiliar code.
...
Alternatively, in the top-down model~\cite{brooks1977towards}, developers maintain a mental model of the system based on their domain knowledge and update that mental model as they discover new evidence. 
...
Our findings suggest that program comprehension plays an important role in defect resolution, because developers' defect resolution strategies, especially those we describe in Sections \ref{cbf} and \ref{acu}, are partly about about trying to understand code.
Indeed, to resolve some defects, developers must explore both familiar and unfamiliar code.
However, other strategies we identified, such as those pertaining to modifying code and interacting with tools, seem beyond the scope of program comprehension.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{Headshots/Heather}}]{Heather Richter Lipford}
	Heather Richter Lipford is an Professor at the University of
	...
	\url{https://webpages.uncc.edu/richter/}
\end{IEEEbiography}
