...
Unsurprisingly, some of the questions previously identified as general programming questions also occur while developers assess security vulnerabilities (Sections \ref{cf} and \ref{dsf} for example).
...
Stars denote previous use of security oriented tools.
...
However, the method is only ever executed as a utility from within the unit test framework. On average, participants spent 14 minutes and 49 seconds on this task.
...
On average, participants spent 8 minutes and 52 seconds on this task.
...
On average, participants spent 13 minutes and 19 seconds on this task.

\noindent\textbf{Task 4} \\
In the method associated with Task 4, a SQL Statement object is created using string interpolation, which is potentially vulnerable to SQL injection. FSB reccomends using  \CodeIn{PreparedStatements} instead. On average, participants spent 8 minutes on this task.
...
\begin{figure*}
	\centering
	\includegraphics[trim={0 0 0 3mm},clip, width=7.5in]{Images/QuestionMerging}
...
%Internal
We faced the internal threat of measuring too many questions. 
That is, recording questions that participants asked because they lacked a basic familiarity with the study environment. 
