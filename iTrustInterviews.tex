...
To answer RQ2 (identify the successful and unsuccessful strategies participants used to meet their information needs) the first author performed two additional passes through the transcripts and recordings. 
%Pass 1
...
Such actions include invoking additional tools, modifying the source code, and searching the web, for example.
%Granularity? Is an action a click? Sometimes. 

To represent these defect resolution strategies we drew from the notion of attack trees [CITE]. 
...
Much like attack trees, our notation organizes actions hierarchically.
We refer to this notation as a strategy tree.
We use strategy trees to represent the hierarchical set of actions a developer takes to resolve a security defect. Figure XX depicts an example strategy tree...
...
%Only consider assumption developers explictly made to us.

What we consider an assumption. Looking for explicit assumptions:
- Keywords (Assume, assuming, presume, reckon, believe, ...)
Should we attempt to identify implicitly held assumption? 
- Trial run 
- 

Implicit assumptions are pervasive in texts~\cite{fairclough2003analysing}; 
to enumerate all the implicit assumptions would be intractable.
We consider only the assumptions that are explicitly triggered within the text. 
Tried to enumerate implicit assumptions, but so . Infinite number of assumptions them. Give plausible examples... 
Assumption was valid, invalid, cannot determine... relate to things outside the scope of the experiment.
...
!StrategyNotes
\begin{itemize}
	\item Build on first paragraph here (what attacks are possible)
	\item Strategies included reading vulnerability info, searching on SO, drawing from prior knowledge
	\item Prone to two types of failures. Either: 
	\item Consider attacks that were not feasible in the context (incorrect)
	\item Determine one type of attack has been guarded against, but don't consider other attacks (incomplete)
\end{itemize}
...
!StrategyNotes
\begin{itemize}
	\item Even when FSB informed participants about a specific alternative, participants leaned heavily on external (not the tool) resources. (Secure Random)
...
!StrategyNotes
\begin{itemize}
	\item Look for ways to apply the fix automatically
	\item None of notifications included quick fixes. Some unsuccessfully attempted to apply quick fixes by clicking on various ui elements
	\item One proposed unnecessary changes, another proposed changes that would introduce another error
	\item Heuristic used was if the bug icon goes away. 
\end{itemize}
...
!StrategyNotes
\begin{itemize}
	\item When looking for information within the project, open files and scroll or scroll through already open files.
	\item unassisted search. Participants 8 scrolled (P2 - P9)
	\item eclipse provides tools for locating some types of information - declaration, type hierarchy, call hierarchy. When participants were looking for that information, scrolling often preempted use of more sophisticated tools
\end{itemize}
...
!StrategyNotes
\begin{itemize}
	\item To answer questions in this category, participants relied primarily on thier prior knowledge of the system
	\item Knowledge of architectural decisions, or experiences on teams contributing to the project
	\item Few strategies to concretely substantiate their existing knowledge
	\item as a result, Failures like P2 in V1 DBbulder is not called on startup
	\item Consult teammates as a strategy 
	\item Already wrote about not surprised issues here
	\item Also comes up in the flow navigation section
...
!StrategyNotes
\begin{itemize}
	\item Determine if it's a test method/ if the user has been authenticated 
	\item Look for keyword indicators. Class name, random password
	\item Not much here for strategies
...
!StrategyNotes
\begin{itemize}
	\item Thinking about the problem on an architectural? level rather than implementation level
	\item Strategies for answering these questions included some of the strategies we already discussed
	\item Thorough strategies included navigating dataflow and control flow, locating information
	\item Othertimes relied on knowledge of code background
...
\begin{itemize}
	...
	\item Copy and paste terms from the notification into google. Iteratively refine search terms. Add the term `java' (P1 V2)
	\item SO offline for maintenance for one participant
	\item Click links in the notification (some hesitated to, because it wasn't clear what concepts they pertained to)	
...
!StrategyNotes
\begin{itemize}
	\item Tool selection strategies:
	\item Often they know what they want the tool to do, but don't know where to find it
	\item right click within the code, or search through menus. 
	\item Failed when participants didn't recognize the tool they were looking for V1 P2
	\item Evaluating selected tool: Unnecessarily switch views and open/close/reopen the same tool
...
\begin{itemize}
	...
	\item Not much else for strategies
