...
Unfortunately, despite their overwhelming availability, research suggests that developers do not use static analysis tools. 
Developers underuse static analysis tools, partially because the tools provide information that does not adequately align with their information needs~\cite{johnson2013don}. 

%Other have categorized information needs
Although existing work has broadly categorized developers' information needs, including the notion that developers need information about their code's security~\cite{latoza2010hard}, no prior study has specifically investigated developers' information needs while using tools to diagnose security vulnerabilities. 
...
Furthermore, in non-security domains, work that identifies information needs has helped toolsmiths both evaluate the effectiveness of existing tools~\cite{ammar2012empirical}, and improve the state of program analysis tools~\cite{kononenko2012automatically, servant2012history, yoon2013visualization}. 
...
To that end, we conducted a laboratory experiment with ten developers who had contributed to iTrust~\cite{iTrust}, a security-critical Java medical records software system.
...
Similarly, Livshits and Lam evaluated their own approach to security-oriented static analysis; specifications provided by the user create static analyzers~\cite{livshits2005finding}. 
...
Further, these studies do not evaluate whether the tools answer questions developers have about the code or vulnerabilities. 
...
\label{questions}
Several studies have explored the developers' information needs.
...
In contrast, our study focuses specifically on the information needs of developers when assessing security vulnerabilities.
...
To identify eligible participants, we recruited via personal contacts, class rosters, and asked participants at the end of the experiment to recommend other qualified participants.
...
Some participants clicked FSB's expecting one type of information, but finding another. 
...
One threat is that some participants had not recently worked with iTrust at the time of the experiment. 
...
\section{Conclusion}
\label{sec:concl}
This paper reported on an experiment conducted to discover developers' information needes while assessing security vulnerabilities in software code.
