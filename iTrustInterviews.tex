...
Unfortunately, however, research suggests that developers are not using static analysis tools, in part because tools do not always effectively address developers' information requirements~\cite{johnson2013don}. 
...
To constrain our study to approximately 60 minutes, we asked each participant to assess four vulnerabilities. 
...
First, participants used error-prone strategies. 
%%Cut this example
For example, in Section \ref{flowNav} we discussed many of the error-prone strategies participants used while navigating program flow.
Participants also used error-prone strategies even when more reliable tools and strategies were available. 
%Either replace this example or rework
For example, in Section \ref{rd}, we noted that participants, unaware of the relevant hyperlinks embedded within the notification text, searched for links to external resources using web search tools. Whereas the web searches returned irrelevant and untrusted results, the embedded links often contained exactly the information participants were searching for.
...
%Replace with different example
For example, some participants wanted to traverse the call hierarchy, but were unaware of the \textsc{call hierarchy} tool and instead manually searched for method calls.
...
%Why was this task complicated
For example, the Servlet Parameter vulnerability introduced the possibility of several types of attacks. 
Participants had to reason about each of those attacks individually and remember which attacks they had ruled out.
In a more general programming context, researchers have warned about the risks of burdening developers' memories~\cite{parnin2012programmer} 
--- interrupting programmers can cause memory failures and the loss of important information.

%Wordy start. Maybe phrase as an approach that makes strategies concrete
We envision an approach that addresses these limitations through hierarchically structured notifications based on checklists.
Previous research suggests that the checklist structure can effectively guide developers~\cite{phang2009triaging}.
We propose a structure that contains hierarchical, customizable tasks for each type of notification.
For example the notification would contain high-level tasks, such as ``Determine which attacks are feasible'', and subsequently more actionable nested subtasks, such as ``Determine if a SQL injection attack is feasible'' or ``Determine if an XSS attack is feasible''.
Our notification structure would also include a checklist-like feature that allows users to save the state of their interaction with a particular notification --- for example, checking off which attack vectors they have already ruled out --- diminishing the potential loss from an interruption.
Additionally, each task could include to resources that relate specifically to that task and tools suggestions that could help developers complete the task.
