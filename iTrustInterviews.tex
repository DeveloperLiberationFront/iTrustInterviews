...
Due to their large number, grouped the categories organize and facilitate discussion about our findings. 
...
%False positives 
Accordingly, developers could benefit from tool support in assessing the implications of suggested fixes.
...
\noindent\textbf{Observations.}
Nine of the ten participants asked questions about the background of the code; most participants wanted to know what parts of the code did.
...
For example, as P6 started exploring the validity of the SQL Injection vulnerability, he wanted to know if he fully understood the error message before he started exploring, so he went back to reread the error message before investigating further.
\\
...
For example, tools could nest definitions and explanations inside collapsible windows within the error message text, next to the concept it relates to. 
...
Perhaps if tools had access to models that represent a developer's conceptual knowledge, similar to work done by Fritz and his colleagues to model developer familiarity with source code~\cite{fritz2010degree}, tools could present additional information only to developers that needed it.
...
He wanted to know if applying the suggested solution would do what he expected it to do. 
Tool support for answering this question might allow the developer to experiment with the fix; the tool could go a step further and run the tests for the code after application to show the developer what the fix does.
...
One participant wanted to know if some code written by the project's developers worked as he expected.
...
However, developers may still have difficulty answering some of the questions they have when using existing tools~\cite{ko2004designing, khoo2008path, johnson2013don}. 
...
During the experiment, we asked ten software developers to walk us through their thoughts as they assessed potential security vulnerability found in iTrust, a medical records web application.
