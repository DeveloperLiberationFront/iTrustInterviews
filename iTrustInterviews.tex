...
Because implicit assumptions are so pervasive ~\cite{fairclough2003analysing}, it would be intractable to enumerate all of them. 
...
Instead of speculating about all of these assumptions, we only consider the assumptions that are explicitly triggered within the transcripts. 
...
To filter only the assumptions participants made about the task or the code, first author inspected each search result.
...
We determined this assumption was incorrect by locating a vulnerability in the library using the common vulnerability and exposures database.\footnote{cve.mitre.org} 
