...
Results of our study suggest that developers may lack support for reasoning about attack surfaces, systematically assessing potential attacks, and locating security-critical components in the code.
...
However, the questions in this category also suggest that developers operate in two distinct modes while reasoning about potential attacks.
At times, they inquire about all the possible attacks that could occur.
Other times, they explore the feasibility of a specific attack.
...
For example, when considering the use of \CodeIn{java.security.SecureRandom} to resolve the Predictable Random vulnerability, participants questioned the applicability of the fix and the consequences of making the change. 
...
While exploring the Predictable Random vulnerability, many participants, including P1, P2, and P6 decided to use \CodeIn{java.security.SecureRandom} without considering any alternative solutions, even though the use of that suggested fix reduces performance.
\\
...
\noindent\textbf{Tool Implications.}
To assure the security of complex software systems developers must reason critically about the code. 
...
%False positives 
Accordingly, developers could benefit from tool support for assessing the implications of suggested fixes.
...
Such a tool might compare the time it takes the code to execute before and after applying the fix.
...
However, in the case of context-dependent questions, developers could benefit from increased tool support. 
For example, if search tools had knowledge of the security-critical components in the code, they could prioritize results which occurred within or interacted with critical components.
Currently, developers must manually prioritize search results and determine whether each is security-relevant.

Developers could also benefit from tool support for searching their applications for code written similarly to the potentially vulnerable code. 
...
Such a tool would help developers identify other parts of the project that may contain vulnerabilities or, conversely, strategies for resolving the vulnerability.
...
Tools like \textsc{find references} and \textsc{call hierarchy} provide support for locating call information, yet leave room for extension.
...
Despite the existence of tools like \textsc{find references} and \textsc{call hierarchy}, developers seem to face barriers preventing them from invoking sophisticated tools, perhaps due to a lack of awareness.
...
A tool that addresses these needs might detect when the developer is looking for call information (e.g., using the \textsc{text search} tool) and suggest the appropriate tool for finding the information they require. 
...
\noindent\textbf{Tool Implications.}
Similar to questions asked about code background and functionality, it is unclear what tool support for answering these kinds of questions would look like, because it was not always obvious why participants wanted these bits of information. 
...
Yet, it may be that participants merely wanted to know the answers to these questions with no high level goal in mind.
...
\noindent\textbf{Tool Implications.}
Research exists that studies attack surfaces, or the sum of the vectors that an unauthorized user may use to gain access to the system.
...
\noindent\textbf{Tool Implications.}
Though there is no one solution to help developers answer these questions, tools that track developers' previous vulnerability encounters may begin to guide to the answers. 
...
However, some participants did not use them because they did not know they existed until the first author pointed them out; as a result, they usually did a web search to find more information.

Tools could provide the desired information by placing links in more visible areas.
For instance, tools could nest definitions and explanations inside collapsible windows within the error message text, next to the concept it relates to. 
One potential downfall to this solution is that developers who understand the concepts will likely not use the links, and maybe even find them distracting.   
Perhaps if tools had access to models that represent a developer's conceptual knowledge, similar to work done by Fritz and his colleagues to model developer familiarity with source code~\cite{fritz2010degree}, tools could present additional information only to developers who needed it.
...
As they build and alter mental models, developers may seek confirmation that they fully understand the code and its intended function. 
For a subset of our participants, this was the case. 
We only identified one distinct question, however, based on our observations, there are at least two ways this question can be interpreted and each requires tool support to answer.
%automatic resolution
One participant located a possible solution from a web resource. 
He wanted to know if applying the suggested solution would do what he expected. 
Tool support for answering this question might allow the developer to experiment with the fix; the tool could go a step further and run the tests for the code after application to show the developer what the fix accomplishes.

%easy access to code documentation 
The other participants sought documentation on how they the code works. 
Answering this question in this context is relatively simple when documentation is available.
One participant wanted to know if some code written by the project's developers worked as he anticipated.
While investigating this, he hovered over the method to view the javadoc, but because one did not exist, he had to use \textsc{open declaration} to find the answer.
Perhaps a tool that uses patterns and elements names in the code to automatically generate documentation could help developers in these  situations.
For example, if the method in question is called \CodeIn{validate()} and takes a String as input and has a condition with an error message or thrown exception, documentation could be generated to communicate that the method is used to validate string input without the developer having to figure it out.
...
It seems the reliability of the resource would be especially important for making changes based on the resource in a secure system.
...
Tools might consider including information about the reliability of the resources provided; one way to do this is to associate the link with peers who have visited it.
Moreover, to help answer questions about other team members' knowledge of the code, tools may be able to borrow ideas from previous research.
...
\noindent\textbf{Tool Implications.}
Developers' ability to understand and effectively interact with tools has been the focus of many research studies. 
Yet, developers may still have difficulty answering some of the questions they have when using existing tools~\cite{ko2004designing, khoo2008path, johnson2013don}. 
...
For example, instead of FSB using phrases and numbers to represent severity, priority, and rank, perhaps FSB could use one scale in which one value can tell you how important the vulnerability is in comparison to the space of other vulnerabilities.
...
\noindent\textbf{Tool Implications.}
Developers asked about the connections between the error message and the code.
...
%present
\item supporting systematic reasoning about potential attack
%application of fix
...
\item using simple one-dimensional metrics for prioritizing vulnerabilities
%
